...
class PDBot(BaseBot):
    ...
    def pd_update_state(self, msg):
        if msg.get('news'):
            ...
            draws = self.mcmc(self.init_prices + [1.5 for x in range(len(self.sources))], unique_qs, 6000, 500)
    ...
    def process(self, msg):
        ...
        for i in range(len(self.sources)):
            ...
            estims = np.array([release.body for release in self.news_releases if release.source == self.sources[i]])
            if len(estims) > 1:
                print np.mean(np.diag(np.cov(estims, rowvar = 0)))

        return None



class PDBotWithQ(BaseBot):

    # If you want to keep track of any information in
    # addition to that stored in BaseBot, feel free to
    # update this init function as necessary.
    def __init__(self, true_qs):
        super(PDBotWithQ, self).__init__()
        self.news_releases = []
        ...
        self.source_to_id = {u"@ETFGodfather" : 0,
                             u"Buzzfeed" : 1,
                             u"The Associated Press" : 2,
                             u"Seeking Alpha" : 3}
    ...
    def likelihood(self, theta):
        ...
        lik = np.zeros(len_news + len_p)
        for i, release in enumerate(self.news_releases):
            try:
                lik[i] = math.log(stats.multivariate_normal.pdf(release.body, 
                                                mean = posited_prices, 
                                                cov = math.pow(self.true_qs[self.source_to_id[release.source]], 2) * self.corr * (600 - float(release.time)) / 60))
            except:
                lik[i] = -99999.
    ...
    def mcmc(self, start, iterations, burn):
        draws = np.zeros((iterations + 1, 5)) 
        draws[0, :] = start
        accept = 0
        for i in range(iterations):
            proposal = np.random.multivariate_normal(mean = draws[i, :5], cov = 0.2 * np.diag([1,1,1,1,1]))
    ...
    # you want to keep track of your internal state.
    def update_state(self, msg):
        super(PDBotWithQ, self).update_state(msg)
    ...
    def pd_update_state(self, msg):
        if msg.get('news'):
            ...
            if msg['news']['time'] < 10:
                return
            draws = self.mcmc(self.init_prices, 6000, 500)
            colmeans = np.mean(draws, axis = 0)
            colses = np.std(draws, axis = 0)
            self.true_prices = list(colmeans[:5])
            self.ses = list(colses[:5])
    ...
    # execute a strategy.
    def process(self, msg):
        super(PDBotWithQ, self).process(msg)
        if msg is not None:
            self.pd_update_state(msg)
        os.system('cls' if os.name == 'nt' else 'clear')
        print(str(self.accept) + "\n")
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 1:
        ...
        bot = PDBotWithQ([float(sys.argv[x]) for x in range(1, 5)])
